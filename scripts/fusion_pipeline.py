...
def render_fusion_video(
    ...
    bev_detections:    "pd.DataFrame | None" = None,
):
    """
...
def process_single_measurement(args, measurement_folder: Path):
    ...
    Full pipeline for one measurement folder:
      ...
      5. Map camera detections into the radar BEV frame via map_detections_to_radar_bev()
...
# ==============================================================================
# CAMERA-TO-RADAR BEV MAPPING
# ------------------------------------------------------------------------------
# Estimate the position and size of each camera detection in the radar BEV
# frame. This produces object-level coordinates that can feed downstream
# model training or radar-centric analysis workflows.
# ==============================================================================
def map_detections_to_radar_bev(detections_df, radar_synched=None):
    """
    Transforms camera bounding boxes to the Bird's-Eye View (BEV) radar coordinate frame.
    Utilizes class-based physical priors and applies 'Radar Snapping' (associating 
    optical detections with dense radar clusters) to correct homography drift.
    """
    df = detections_df.copy()
    if 'cam_X' not in df.columns or 'cam_Y' not in df.columns:
        logging.warning("BEV projection skipped: cam_X/cam_Y missing from homography.")
        return df

    # 1. Base coordinates from camera homography
    df['rad_X'] = df['cam_X']
    df['rad_Y'] = df['cam_Y']
    
    # 2. Physical Extent Priors (rad_W, rad_L)
    priors = {
        0: (0.6, 0.6),  # Pedestrian
        1: (0.6, 1.8),  # Bicycle
        2: (1.8, 4.5),  # Car
        3: (0.8, 1.8),  # Cyclist
    }
    ...
    # 3. Sensor Fusion: Target Snapping
    if radar_synched is not None and not radar_synched.empty:
        ...
        for idx, row in df.iterrows():
            ...
            # L2 distance to radar points
            dist = np.sqrt((r_pts['X'] - row['cam_X'])**2 + (r_pts['Y'] - row['cam_Y'])**2)
            
            # Local cluster association (Radius = 2.5m)
        ...
        logging.info(f"Sensor Fusion: Associated {snapped_count}/{len(df)} optical tracks with radar signatures.")
