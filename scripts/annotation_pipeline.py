...
def create_custom_tracker(output_path: str, args: argparse.Namespace) -> str:
    ...
    Provides all keys to prevent Ultralytics AttributeError crashes on newer versions.
    """
    config = {
        ...
        # Default fallback keys to prevent Ultralytics backend crashes
...
def run_inference(measurement_folder, model: YOLO, tracker_yaml: str | None, args: argparse.Namespace):
    ...
    Handles device allocation and logs model architecture metadata.
    """
    ...
    # Log model metadata for the selected pretrained weights.
    logging.info(f"\n[MODEL INFO] Architecture details for {args.model}:")
...
def compute_homography(ref: pd.DataFrame, args: argparse.Namespace):
    """
    Computes Direct Linear Transformation (DLT) homography mapping 
    camera pixels (u, v) -> BEV world coordinates (x_m, y_m).
    """
    src = ref[["u", "v"]].to_numpy(dtype=np.float64)
    dst = ref[["x_m", "y_m"]].to_numpy(dtype=np.float64)
    
    # method=0 forces standard DLT
    H, _ = cv2.findHomography(src, dst, method=0)
    
    if H is None:
        raise RuntimeError("findHomography (DLT) failed. Check point correspondences.")
        
    # Log reprojection error for sanity checking
    src_hom = np.hstack([src, np.ones((src.shape[0], 1))])
    pred_hom = (H @ src_hom.T).T
    pred = pred_hom[:, :2] / pred_hom[:, 2:]
    mean_err = np.mean(np.linalg.norm(dst - pred, axis=1))
    
    logging.info(f"[HOMOGRAPHY] DLT Matrix Computed. Mean Reprojection Error: {mean_err:.4f} m")
...
# ==============================================================================
# Camera-site calibration data: known parked-car pixel centers for each
# measurement period at the deployed traffic camera location.
...
#
# NOTE: These coordinates are camera-site specific and should be updated
# or cleared when the installation or measurement geometry changes.
_PARKED_CARS = {
...
# ==============================================================================
# CYCLIST DETECTION MERGE
# ------------------------------------------------------------------------------
# YOLO can detect the bicycle frame (cls_id=1, 'bic') and the rider
# (cls_id=0, 'ped') as two separate boxes on the same frame. For dataset
# labelling and radar fusion, a single cyclist detection enclosing both
# objects is produced.
# ==============================================================================
